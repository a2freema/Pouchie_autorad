...
load_pkg <- rlang::quos(tidyverse, ade4, ade4TkGUI, exactRankTests, ggplot2, stargazer, ggthemes, lme4, lmerTest, emmeans, extrafont, car, corrplot, tidyverse, ggpubr, osfr, broom, vegan)
...
```{r pouchie ot boxplot, echo=FALSE, warning=FALSE, fig.width= 4, fig.height=7}
...
```{r pouchie v1 boxplot, echo=FALSE, warning=FALSE, fig.width= 4, fig.height=7}
...
figv1MS <- ggbiplot(fitv1MS, obs.scale = 1, var.scale = 1, 
               ellipse = F, labels=fullname, groups=mygenus,
               label.size = 5, varname.size = 2,
               circle = F)
figv1MS <- figv1MS  + scale_x_continuous(limits=c( -4, 4.5))
figv1MS <- figv1MS + scale_y_continuous(limits=c(-2.5, 3))
figv1MS <- figv1MS + scale_color_manual(values=(tol11qualitative)) + scale_fill_manual(values=(tol11qualitative))
...
fig2 <- ggbiplot(fitv1MR, obs.scale = 1, var.scale = 1, 
               ellipse = F, labels=fullnameMR, groups=mygenusMR,
               label.size = 5, varname.size = 2,
               circle = F) ## to see the brain regions
fig2 <- fig2  + scale_x_continuous(limits=c(-4, 4.5))
fig2 <- fig2 + scale_y_continuous(limits=c(-2.5, 3))
fig2 <- fig2 + scale_color_manual(values=(tol9qualitative)) + scale_fill_manual(values=(tol9qualitative))
...
![a PCA](Figure v1ar max regions.png)
